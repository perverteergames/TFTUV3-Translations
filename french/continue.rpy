translate french strings:


    old "Please select the Season 2 save you'd like to load:"
    new "Choisis la sauvegarde de la saison 2 que tu veux charger :"


    old "General"
    new "Général"


    old "Story"
    new "Histoire"


    old "Did Thim enter the Pit of Despair on Nyiruan 12-Beta?"
    new "Thim est-il entré dans le Puits du désespoir sur Nyiruan 12-Beta ?"


    old "Did you help the R'o escape the Pit of Despair?"
    new "As-tu aidé les R'o à s'échapper du Puits du désespoir ?"


    old "Did you sign the contract and enter the ConVitæ on Verdant station?"
    new "As-tu signé le contrat et rejoint le ConVitæ sur la station Verdant ?"


    old "Did you have sex with Luzanne (Futa)?"
    new "As-tu couché avec Luzanne (Futa) ?"


    old "Did you execute Garomph in the human milking factory on Barranthis?"
    new "As-tu exécuté Garomph dans l'usine de traite humaine sur Barranthis ?"


    old "Did you sabotage the human milking factory on Barranthis?"
    new "As-tu saboté l'usine de traite humaine sur Barranthis ?"


    old "Did you help the women escape from the human milking factory on Barranthis?"
    new "As-tu aidé les femmes à s'échapper de l'usine de traite humaine sur Barranthis ?"


    old "Did you alter the simulation of Eva and Lilly on the Iron Bastard?"
    new "As-tu modifié la simulation d'Eva et Lilly sur l'Iron Bastard ?"


    old "Did you delete the simulation of Eva and Lilly on the Iron Bastard?"
    new "As-tu supprimé la simulation d'Eva et Lilly sur l'Iron Bastard ?"


    old "Did you abbandon the simulation of Eva and Lilly on the Iron Bastard?"
    new "As-tu abandonné la simulation d'Eva et Lilly sur l'Iron Bastard ?"


    old "Did form an alliance with the Warrior caste on Erigone?"
    new "T'as formé une alliance avec la caste des guerriers sur Erigone ?"


    old "Did form an alliance with the Priest caste on Erigone?"
    new "T'as formé une alliance avec la caste des prêtres sur Erigone ?"


    old "Did you remain neutral and declined to form any alliances on Erigone?"
    new "T'es resté neutre et t'as refusé de former des alliances sur Erigone ?"


    old "Did you rescue the Acarhyn warrior during the Hunt on Erigone?"
    new "T'as sauvé le guerrier Acarhyn pendant la chasse sur Erigone ?"


    old "Are you in a relationship with Lilly?"
    new "T'es en couple avec Lilly ?"


    old "Are you in a relationship with Aven?"
    new "T'es en couple avec Aven ?"


    old "Were trying to get into a relationship with Nadya?"
    new "T'as essayé d'entamer une relation avec Nadya ?"


    old "Are you in a relationship with Céline?"
    new "Tu es en couple avec Céline ?"


    old "Do you want Céline to be part of your future harem?"
    new "Tu veux que Céline fasse partie de ton futur harem ?"


    old "Are you in a relationship with Thyia?"
    new "Tu es en couple avec Thyia ?"


    old "Do you want Thyia to be part of your future harem?"
    new "Tu veux que Thyia fasse partie de ton futur harem ?"


    old "Were trying to be friends with Ziv?"
    new "Tu as essayé d'être ami avec Ziv ?"


    old "Are you in a relationship with Ziv?"
    new "Tu es en couple avec Ziv ?"


    old "Do you want Vess to be your submissive?"
    new "Tu veux que Vess soit ta soumise ?"


    old "Are you in a relationship with Vess?"
    new "Tu es en couple avec Vess ?"


    old "Are you in a relationship with Jade?"
    new "T'es en couple avec Jade ?"


    old "Is Jade your submissive?"
    new "Jade est ta soumise ?"


    old "Were trying to be friends with Raene?"
    new "Tu essaies d'être pote avec Raene ?"


    old "Were trying to get into a relationship with Raene?"
    new "Tu essaies d'avoir une relation avec Raene ?"


    old "Have you befriended Thim?"
    new "T'es devenu pote avec Thim ?"


    old "Misc"
    new "Divers"


    old "Did you participate in the R'o orgy?"
    new "T'as participé à l'orgie des R'o ?"


    old "Did you creampie D'ee, the R'o leader?"
    new "T'as fait un creampie à D'ee, le chef des R'o ?"


    old "Did you creampie the R'o virgin?"
    new "T'as fait un creampie à la vierge des R'o ?"


    old "Did you have sex in the ConVitæ simulation?"
    new "Tu as couché dans la simulation ConVitæ ?"


    old "Did you have sex with Berit in the ConVitæ simulation?"
    new "Tu as couché avec Berit dans la simulation ConVitæ ?"


    old "Did you have sex with Leda in the ConVitæ simulation?"
    new "Tu as couché avec Leda dans la simulation ConVitæ ?"


    old "Did you have sex with the doctor at the ConVitæ research station?"
    new "Tu as couché avec le docteur à la station de recherche ConVitæ ?"


    old "Did you have an orgy with the Acarhyn on Erigone?"
    new "Tu as participé à une orgie avec les Acarhyn sur Erigone ?"


    old "Did Kit join you during the orgy with the Acarhyn on Erigone?"
    new "Kit s'est joint à toi pendant l'orgie avec les Acarhyn sur Erigone ?"


    old "Did you have sex with Yve?"
    new "Tu as couché avec Yve ?"


    old "Fetish"
    new "Fétichisme"


    old "Anal"
    new "Anal"


    old "Rimming"
    new "Anulingus"


    old "Watersports"
    new "Sports nautiques"


    old "Start a new game"
    new "Commencer une nouvelle partie"


    old "The game is best enjoyed when you import a saved game from Season One. If you've lost access to your season saves or just want to begin with Season Two you can select the most important choices from the first season below, all other decisions will be set to a default value."
    new "Pour profiter pleinement du jeu, importe une partie sauvegardée de la saison 1. Si tu n'as plus accès à tes sauvegardes de la saison 1 ou si tu veux simplement commencer la saison 2, tu peux sélectionner ci-dessous les choix les plus importants de la première saison. Toutes les autres décisions seront réglées sur leur valeur par défaut."


    old "Start new game"
    new "Commencer une nouvelle partie"


    old "Do you want to get Lilly pregnant?"
    new "Tu veux mettre Lilly enceinte ?"


    old "Were trying to be friends with Ziv (Futa)?"
    new "Tu essayais de devenir ami avec Ziv (Futa) ?"


    old "Are you in a relationship with Ziv (Futa)?"
    new "Êtes-vous en couple avec Ziv (Futa) ?"


    old "Were trying to be friends with Raene (Trans)?"
    new "Essayais-tu d'être ami avec Raene (Trans) ?"


    old "Were trying to get into a relationship with Raene (Trans)?"
    new "Essayais-tu d'entamer une relation avec Raene (Trans) ?"


    old "No season saves currently available"
    new "Aucune saison enregistrée actuellement disponible"


    old "The game is best enjoyed when you import a saved game from Season Two. If you've lost access to your season saves or just want to begin with Season Three you can select the most important choices from the first and second season below, all other decisions will be set to a default value."
    new "Pour profiter au max du jeu, importe une sauvegarde de la saison 2. Si t'as plus accès à tes sauvegardes ou que tu veux juste commencer la saison 3, tu peux choisir les options les plus importantes des saisons 1 et 2 ci-dessous. Toutes les autres décisions seront réglées sur une valeur par défaut."
