# TODO: Translation updated at 2025-05-18 20:59

translate russian strings:

    # game/continue.rpy:13
    old "Please select the Season 1 save you'd like to load, or start a new game instead:"
    new "Пожалуйста, выберите сохранение из Сезона 1, которое вы хотели бы загрузить, или начните новую игру:"

    # game/continue.rpy:47
    old "Start new game"
    new "Начать новую игру"

    # game/continue.rpy:91
    old "General"
    new "Основное"

    # game/continue.rpy:91
    old "Story"
    new "История"

    # game/continue.rpy:91
    old "Did Thim enter the Pit of Despair on Nyiruan 12-Beta?"
    new "Попал ли Тхим в Яму Отчаяния на Ньируане 12-Бета?"

    # game/continue.rpy:91
    old "Did you help the R'o escape the Pit of Despair?"
    new "Помог ли ты Р'о выбраться из Ямы Отчаяния?"

    # game/continue.rpy:91
    old "Did you sign the contract and enter the ConVitæ on Verdant station?"
    new "Ты подписал контракт и присоединился к ConVitæ на станции Вердант?"

    # game/continue.rpy:91
    old "Did you have sex with Luzanne (Futa)?"
    new "У тебя был секс с Лузан (Фута)?"

    # game/continue.rpy:91
    old "Did you execute Garomph in the human milking factory on Barranthis?"
    new "Ты казнил Гаромфа на человеческой доильной фабрике на Баррантисе?"

    # game/continue.rpy:91
    old "Did you sabotage the human milking factory on Barranthis?"
    new "Ты устроил диверсию на человеческой доильной фабрике на Баррантисе?"

    # game/continue.rpy:91
    old "Did you help the women escape from the human milking factory on Barranthis?"
    new "Ты помог женщинам сбежать с человеческой доильной фабрики на Баррантисе?"

    # game/continue.rpy:91
    old "Did you alter the simulation of Eva and Lilly on the Iron Bastard?"
    new "Ты изменил имитацию с Евой и Лилли на Железном Ублюдке?"

    # game/continue.rpy:91
    old "Did you delete the simulation of Eva and Lilly on the Iron Bastard?"
    new "Ты удалил имитацию с Евой и Лилли на Железном Ублюдке?"

    # game/continue.rpy:91
    old "Did you abbandon the simulation of Eva and Lilly on the Iron Bastard?"
    new "Ты покинул имитацию с Евой и Лилли на Железном Ублюдке?"

    # game/continue.rpy:91
    old "Did form an alliance with the Warrior caste on Erigone?"
    new "Заключил ли ты союз с кастой Воинов на Эригоне?"

    # game/continue.rpy:91
    old "Did form an alliance with the Priest caste on Erigone?"
    new "Заключил ли ты союз с кастой Жрецов на Эригоне?"

    # game/continue.rpy:91
    old "Did you remain neutral and declined to form any alliances on Erigone?"
    new "Ты сохранил нейтралитет и отказался заключать какие-либо союзы на Эригоне?"

    # game/continue.rpy:91
    old "Did you rescue the Acarhyn warrior during the Hunt on Erigone?"
    new "Ты спас воина-Акархина во время Охоты на Эригоне?"

    # game/continue.rpy:91
    old "Are you in a relationship with Lilly?"
    new "Ты состоишь в отношениях с Лилли?"

    # game/continue.rpy:91
    old "Are you in a relationship with Aven?"
    new "Ты состоишь в отношениях с Эвин?"

    # game/continue.rpy:91
    old "Were trying to get into a relationship with Nadya?"
    new "Ты пытался завязать отношения с Надией?"

    # game/continue.rpy:91
    old "Are you in a relationship with Céline?"
    new "Ты состоишь в отношениях с Селин?"

    # game/continue.rpy:91
    old "Do you want Céline to be part of your future harem?"
    new "Ты хочешь, чтобы Селин стала частью твоего будущего гарема?"

    # game/continue.rpy:91
    old "Are you in a relationship with Thyia?"
    new "Ты состоишь в отношениях с Тхией?"

    # game/continue.rpy:91
    old "Do you want Thyia to be part of your future harem?"
    new "Ты хочешь, чтобы Тхия стала частью твоего будущего гарема?"

    # game/continue.rpy:91
    old "Were trying to be friends with Ziv?"
    new "Ты пытался подружиться с Зив?"

    # game/continue.rpy:91
    old "Are you in a relationship with Ziv?"
    new "Ты состоишь в отношениях с Зив?"

    # game/continue.rpy:91
    old "Do you want Vess to be your submissive?"
    new "Хочешь, чтобы Весс была твоей сабмиссив?"

    # game/continue.rpy:91
    old "Are you in a relationship with Vess?"
    new "Ты состоишь в отношениях с Весс?"

    # game/continue.rpy:91
    old "Are you in a relationship with Jade?"
    new "Ты состоишь в отношениях с Джейд?"

    # game/continue.rpy:91
    old "Is Jade your submissive?"
    new "Джейд - твой сабмиссив?"

    # game/continue.rpy:91
    old "Were trying to be friends with Raene?"
    new "Ты пытался подружиться с Рэин?"

    # game/continue.rpy:91
    old "Were trying to get into a relationship with Raene?"
    new "Ты пытался завязать отношения с Рэин?"

    # game/continue.rpy:91
    old "Have you befriended Thim?"
    new "Ты подружился с Тхимом?"

    # game/continue.rpy:91
    old "Misc"
    new "Разное"

    # game/continue.rpy:91
    old "Did you participate in the R'o orgy?"
    new "Ты участвовал в оргии Р'о?"

    # game/continue.rpy:91
    old "Did you creampie D'ee, the R'o leader?"
    new "Ты кончил в Д'ии, лидера Р'о?"

    # game/continue.rpy:91
    old "Did you creampie the R'o virgin?"
    new "Ты кончил в девственницу из Р'о?"

    # game/continue.rpy:91
    old "Did you have sex in the ConVitæ simulation?"
    new "Занимался ли ты сексом в имитации ConVitæ?"

    # game/continue.rpy:91
    old "Did you have sex with Berit in the ConVitæ simulation?"
    new "Занимался ли ты сексом с Берит в имитации ConVitæ?"

    # game/continue.rpy:91
    old "Did you have sex with Leda in the ConVitæ simulation?"
    new "Занимался ли ты сексом с Ледой в имитации ConVitæ?"

    # game/continue.rpy:91
    old "Did you have sex with the doctor at the ConVitæ research station?"
    new "Занимался ли ты сексом с доктором на исследовательской станции ConVitæ?"

    # game/continue.rpy:91
    old "Did you have an orgy with the Acarhyn on Erigone?"
    new "У тебя была оргия с Акархинами на Эригоне?"

    # game/continue.rpy:91
    old "Did Kit join you during the orgy with the Acarhyn on Erigone?"
    new "Кит присоединился к тебе во время оргии с Акархинами на Эригоне?"

    # game/continue.rpy:91
    old "Did you have sex with Yve?"
    new "Ты занимался сексом с Ив?"

    # game/continue.rpy:91
    old "Fetish"
    new "Фетиш"

    # game/continue.rpy:91
    old "Rimming"
    new "Римминг"

    # game/continue.rpy:91
    old "Watersports"
    new "Водные виды спорта"

    # game/continue.rpy:872
    old "Start a new game"
    new "Начать новую игру"

    # game/continue.rpy:886
    old "The game is best enjoyed when you import a saved game from Season One. If you've lost access to your season saves or just want to begin with Season Two you can select the most important choices from the first season below, all other decisions will be set to a default value."
    new "Наибольшее удовольствие от игры можно получить, если импортировать сохранение из Первого Сезона. Если вы потеряли доступ к своим сохранениям или просто хотите начать со Второго Сезона, вы можете выбрать наиболее важные варианты из первого сезона ниже, все остальные решения будут установлены по умолчанию."

# TODO: Translation updated at 2025-10-07 18:13

translate russian strings:

    # game/continue.rpy:14
    old "Please select the Season 2 save you'd like to load:"
    new "Пожалуйста, выберите сохранение 2-го сезона, которое вы хотели бы загрузить:"

    # game/continue.rpy:44
    old "No season saves currently available"
    new "В данный момент сезонные сохранения отсутствуют"

    # game/continue.rpy:95
    old "Do you want to get Lilly pregnant?"
    new "Планируешь ли ты беременность у Лилли?"

    # game/continue.rpy:95
    old "Were trying to be friends with Ziv (Futa)?"
    new "Пытаешься подружиться с Зив (Фута)?"

    # game/continue.rpy:95
    old "Are you in a relationship with Ziv (Futa)?"
    new "Вы с Зив состоите в отношениях (Фута)?"

    # game/continue.rpy:95
    old "Were trying to be friends with Raene (Trans)?"
    new "Пытаешься подружиться с Рэин (Транс)?"

    # game/continue.rpy:95
    old "Were trying to get into a relationship with Raene (Trans)?"
    new "Пытаешься построить отношения с Рэин (Транс)?"

# TODO: Translation updated at 2026-03-09 11:09

translate russian strings:

    # game/continue.rpy:95
    old "Anal"
    new ""

    # game/continue.rpy:924
    old "The game is best enjoyed when you import a saved game from Season Two. If you've lost access to your season saves or just want to begin with Season Three you can select the most important choices from the first and second season below, all other decisions will be set to a default value."
    new ""

