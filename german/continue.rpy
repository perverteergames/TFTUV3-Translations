# TODO: Translation updated at 2025-11-20 00:47

translate german strings:

    # game/continue.rpy:14
    old "Please select the Season 2 save you'd like to load:"
    new "Bitte wähle den Spielstand aus Staffel 2 aus, den du laden möchtest:"

    # game/continue.rpy:44
    old "No season saves currently available"
    new "Derzeit sind keine Saison-Spielstände verfügbar."

    # game/continue.rpy:95
    old "General"
    new "General"

    # game/continue.rpy:95
    old "Story"
    new "Geschichte"

    # game/continue.rpy:95
    old "Did Thim enter the Pit of Despair on Nyiruan 12-Beta?"
    new "Ist Thim auf Nyiruan 12-Beta in die Grube der Verzweiflung eingetreten?"

    # game/continue.rpy:95
    old "Did you help the R'o escape the Pit of Despair?"
    new "Hast du den R\'o geholfen, aus der Grube der Verzweiflung zu entkommen?"

    # game/continue.rpy:95
    old "Did you sign the contract and enter the ConVitæ on Verdant station?"
    new "Hast du den Vertrag unterschrieben und bist du am Bahnhof Verdant ins ConVitæ eingestiegen?"

    # game/continue.rpy:95
    old "Did you have sex with Luzanne (Futa)?"
    new "Hattest du Sex mit Luzanne (Futa)?"

    # game/continue.rpy:95
    old "Did you execute Garomph in the human milking factory on Barranthis?"
    new "Hast du Garomph in der Menschenmelkfabrik auf Barranthis hingerichtet?"

    # game/continue.rpy:95
    old "Did you sabotage the human milking factory on Barranthis?"
    new "Hast du die Menschenmilchfabrik auf Barranthis sabotiert?"

    # game/continue.rpy:95
    old "Did you help the women escape from the human milking factory on Barranthis?"
    new "Hast du den Frauen geholfen, aus der Menschenmelkfabrik auf Barranthis zu fliehen?"

    # game/continue.rpy:95
    old "Did you alter the simulation of Eva and Lilly on the Iron Bastard?"
    new "Hast du die Simulation von Eva und Lilly auf der Iron Bastard verändert?"

    # game/continue.rpy:95
    old "Did you delete the simulation of Eva and Lilly on the Iron Bastard?"
    new "Hast du die Simulation von Eva und Lilly auf der Iron Bastard gelöscht?"

    # game/continue.rpy:95
    old "Did you abbandon the simulation of Eva and Lilly on the Iron Bastard?"
    new "Hast du die Simulation von Eva und Lilly auf der Iron Bastard abgebrochen?"

    # game/continue.rpy:95
    old "Did form an alliance with the Warrior caste on Erigone?"
    new "Hast du ein Bündnis mit der Kriegerkaste auf Erigone geschlossen?"

    # game/continue.rpy:95
    old "Did form an alliance with the Priest caste on Erigone?"
    new "Hast du ein Bündnis mit der Priesterkaste auf Erigone geschlossen?"

    # game/continue.rpy:95
    old "Did you remain neutral and declined to form any alliances on Erigone?"
    new "Bist du neutral geblieben und hast es abgelehnt, auf Erigone Bündnisse einzugehen?"

    # game/continue.rpy:95
    old "Did you rescue the Acarhyn warrior during the Hunt on Erigone?"
    new "Hast du den Acarhyn-Krieger während der Jagd auf Erigone gerettet?"

    # game/continue.rpy:95
    old "Are you in a relationship with Lilly?"
    new "Bist du mit Lilly zusammen?"

    # game/continue.rpy:95
    old "Do you want to get Lilly pregnant?"
    new "Möchtest du, dass Lilly schwanger wird?"

    # game/continue.rpy:95
    old "Are you in a relationship with Aven?"
    new "Bist du mit Aven in einer Beziehung?"

    # game/continue.rpy:95
    old "Were trying to get into a relationship with Nadya?"
    new "Versuchst du, eine Beziehung mit Nadya anzufangen?"

    # game/continue.rpy:95
    old "Are you in a relationship with Céline?"
    new "Bist du mit Céline zusammen?"

    # game/continue.rpy:95
    old "Do you want Céline to be part of your future harem?"
    new "Möchtest du, dass Céline Teil deines zukünftigen Harems wird?"

    # game/continue.rpy:95
    old "Are you in a relationship with Thyia?"
    new "Bist du mit Thyia in einer Beziehung?"

    # game/continue.rpy:95
    old "Do you want Thyia to be part of your future harem?"
    new "Möchtest du, dass Thyia Teil deines zukünftigen Harems wird?"

    # game/continue.rpy:95
    old "Were trying to be friends with Ziv (Futa)?"
    new "Versuchen wir, mit Ziv (Futa) befreundet zu sein?"

    # game/continue.rpy:95
    old "Are you in a relationship with Ziv (Futa)?"
    new "Bist du mit Ziv (Futa) zusammen?"

    # game/continue.rpy:95
    old "Do you want Vess to be your submissive?"
    new "Möchtest du, dass Vess deine Submissive wird?"

    # game/continue.rpy:95
    old "Are you in a relationship with Vess?"
    new "Bist du mit Vess in einer Beziehung?"

    # game/continue.rpy:95
    old "Are you in a relationship with Jade?"
    new "Bist du mit Jade zusammen?"

    # game/continue.rpy:95
    old "Is Jade your submissive?"
    new "Ist Jade deine Submissive?"

    # game/continue.rpy:95
    old "Were trying to be friends with Raene (Trans)?"
    new "Versuchen wir, mit Raene (Trans) befreundet zu sein?"

    # game/continue.rpy:95
    old "Were trying to get into a relationship with Raene (Trans)?"
    new "Versuchen Sie, eine Beziehung mit Raene (Trans) einzugehen?"

    # game/continue.rpy:95
    old "Have you befriended Thim?"
    new "Hast du dich mit Thim angefreundet?"

    # game/continue.rpy:95
    old "Misc"
    new "Verschiedenes"

    # game/continue.rpy:95
    old "Did you participate in the R'o orgy?"
    new "Hast du an der R\'o-Orgie teilgenommen?"

    # game/continue.rpy:95
    old "Did you creampie D'ee, the R'o leader?"
    new "Hast du D\'ee, den R\'o-Anführer, mit Sperma bespritzt?"

    # game/continue.rpy:95
    old "Did you creampie the R'o virgin?"
    new "Hast du die Jungfrau von R\'o mit Sperma bespritzt?"

    # game/continue.rpy:95
    old "Did you have sex in the ConVitæ simulation?"
    new "Hattest du Sex in der ConVitæ-Simulation?"

    # game/continue.rpy:95
    old "Did you have sex with Berit in the ConVitæ simulation?"
    new "Hattest du Sex mit Berit in der ConVitæ-Simulation?"

    # game/continue.rpy:95
    old "Did you have sex with Leda in the ConVitæ simulation?"
    new "Hattest du Sex mit Leda in der ConVitæ-Simulation?"

    # game/continue.rpy:95
    old "Did you have sex with the doctor at the ConVitæ research station?"
    new "Hattest du Sex mit dem Arzt an der ConVitæ-Forschungsstation?"

    # game/continue.rpy:95
    old "Did you have an orgy with the Acarhyn on Erigone?"
    new "Hattest du auf Erigone eine Orgie mit den Acarhyn?"

    # game/continue.rpy:95
    old "Did Kit join you during the orgy with the Acarhyn on Erigone?"
    new "War Kit bei der Orgie mit den Acarhyn auf Erigone dabei?"

    # game/continue.rpy:95
    old "Did you have sex with Yve?"
    new "Hattest du Sex mit Yve?"

    # game/continue.rpy:95
    old "Fetish"
    new "Fetisch"

    # game/continue.rpy:95
    old "Anal"
    new "Anal"

    # game/continue.rpy:95
    old "Rimming"
    new "Rimming"

    # game/continue.rpy:95
    old "Watersports"
    new "Wassersport"

    # game/continue.rpy:910
    old "Start a new game"
    new "Starte ein neues Spiel"

    # game/continue.rpy:924
    old "The game is best enjoyed when you import a saved game from Season One. If you've lost access to your season saves or just want to begin with Season Two you can select the most important choices from the first season below, all other decisions will be set to a default value."
    new "Das Spiel macht am meisten Spaß, wenn du einen Spielstand aus Staffel 1 importierst. Falls du den Zugriff auf deine Spielstände verloren hast oder einfach mit Staffel 2 beginnen möchtest, kannst du unten die wichtigsten Entscheidungen aus der ersten Staffel auswählen. Alle anderen Entscheidungen werden auf Standardwerte zurückgesetzt."

    # game/continue.rpy:1003
    old "Start new game"
    new "Starte ein neues Spiel"

# TODO: Translation updated at 2026-03-09 11:09

translate german strings:

    # game/continue.rpy:924
    old "The game is best enjoyed when you import a saved game from Season Two. If you've lost access to your season saves or just want to begin with Season Three you can select the most important choices from the first and second season below, all other decisions will be set to a default value."
    new ""

